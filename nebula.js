  ...
  clientio.on("connection", function (socket) {
    ...
    socket.on("list.sessions", function () {
      socket.emit(
        ...
        Object.fromEntries(clientio.sockets.adapter.rooms)
      );
    });
    ...
    function disconnectClient() {
      ...
      if (roomData && roomData.hasOwnProperty("count")) {
        ...
        if (roomData.count <= 0) {
          ...
          var usedSessionNums2 = usedSessionNumbers.slice(
            sessionNumIndex + 1,
            usedSessionNumbers.length
          );
    ...
    */
    var csvFileReady = function (csvFilePath) {
      ...
      rl.on("error", function (err) {
        console.log(
          socket.roomName + ": Error while parsing CSV file: " + csvFilePath
        );
        console.log(err);
      });
      ...
      rl.on("close", function () {
        ...
        if (
          (lastObservationKeys.length =
            1 && lastObservation[lastObservationKeys[0]] == "")
        ) {
          csvData.pop();
        }
    ...
    */
    socket.on("setData", function (data, room) {
      ...
      var childProcess = exec(command, "-e", function (error, stdout, stderr) {
        ...
        if (stdout) {
          console.log(
            socket.roomName + ": Creating CSV file stdout: " + stdout
          );
        }
        ...
        if (stderr) {
          console.log(
            socket.roomName + ": Creating CSV file stderr: " + stderr
          );
        }
      });
    ...
    */
    socket.on("getSessionName", function (ui) {
      ...
      if (
        ...
        nextSessionNumber + 1 > Number.MAX_VALUE
      ) {
        ...
        nextSessionNumber = 0;
        while (
          usedSessionNumber.indexOf(nextSessionNumber) >= 0 &&
          nextSessionNumber < Number.MAX_VALUE
        ) {
          nextSessionNumber++;
        }
    ...
    */
    ...
    function deleteFile(filePath) {
      fs.stat(filePath, function (err, data) {
        if (err) {
          console.log(
            socket.roomName + ": File " + filePath + " does not exist"
          );
        } else {
          fs.unlink(filePath, function (err) {
            if (err) {
              return console.error(
                socket.roomName + ": Error unlinking file: " + err
              );
            }
          });
        }
          ...
          if (!pipelineAddr) {
            ...
            pipelineInstance.on("error", function (err) {
              ...
              pipelineInstance.stderr.on("data", function (data) {
                console.log(
                  socket.roomName + " Pipeline error: " + data.toString()
                );
              });
            });
            ...
            */
            ...
            pipelineInstance.stdout.on("data", function (data) {
              console.log(
                socket.roomName + " Pipeline STDOUT: " + data.toString()
              );
            });
            pipelineInstance.stderr.on("data", function (data) {
              console.log(
    ...
    socket.on("action", function (data, isObservation) {
      if (socket.room) {
        ...
        if (typeof isObservation == "undefined") {
          ...
          socket.broadcast
            .to(socket.roomName)
            .emit("action", data, isObservation);
        }
      }
    });
          ...
          if (data.type === "oli") {
            if (typeof isObservation == "undefined") {
              invoke(socket.room.pipelineSocket, "update", {
                interaction: "oli",
                type: "classic",
                points: oli(socket.room),
              });
            } else {
              invoke(socket.room.pipelineSocket, "update", {
                interaction: "oli",
                type: "classic",
                points: oli(socket.room, isObservation),
                docFeedback: obsFeedback,
                attrFeedback: attrFeedback,
                docForage: obsForage,
                attrForage: attrForage,
                view: isObservation,
                prototype: prototype,
              });
            }
...
Nebula.prototype.handleAction = function (action, room) {
  if (action.type === "move") {
    if (room.points.has(action.id)) {
      ...
      typeof room.attribute_points != "undefined" &&
      room.attribute_points.has(action.id)
    ) {
      room.attribute_points.get(action.id).pos = action.pos;
    } else {
      console.log(
        room.name + ": Point not found in room for move: " + action.id
      );
    }
    ...
    if (room.points.has(action.id)) {
      ...
      if (typeof socket != "undefined") {
        console.log(
          room.name + ": Point not found in room for select: " + action.id
        );
      } else {
        console.log(
          "Point not for for room in select... " +
    ...
    if (room.points.has(action.id)) {
      ...
      console.log(
        room.name + ": Point not found in room for sampling: " + action.id
      );
    }
  }
};
...
Nebula.prototype.handleMessage = function (room, msg) {
  ...
  if (obj.func) {
    if (obj.func === "update") {
      ...
      invoke(room.pipelineSocket, "update", {
        interaction: "none",
        prototype: sirius_prototype,
      });
    }
  }
};
    ...
    if (update.points) {
      for (var i = 0; i < update.points.length; i++) {
        ...
        if (room.attribute_points.has(point.id)) {
          if (point.pos) room.attribute_points.get(point.id).pos = point.pos;
          if (point.relevance)
    ...
    if (update.similarity_weights) {
      for (var i = 0; i < update.similarity_weights.length; i++) {
        ...
        if (room.attribute_similarity_weights.has(weight.id)) {
          room.attribute_similarity_weights.get(weight.id).weight =
            weight.weight;
        } else {
...
var sendRoom = function (room, isObservation) {
  ...
  if (typeof isObservation == "undefined" || isObservation) {
    ...
    modRoom.similarity_weights = Array.from(
      room.attribute_similarity_weights.values()
    );
  }
  return modRoom;
};
